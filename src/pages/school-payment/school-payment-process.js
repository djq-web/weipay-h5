...
class SchoolPaymentProcess extends Component {
  constructor(props) {
    super(props);
    this.state = {
      orderList: [],
  ...
  queryOrderDetail=()=> {
    ...
    Post(API.querySchOrderDetail, parmas, true).then((res) => {
         ...
         { orderList.map((item,index) => {
          ...
          <div className="process-bottom">
            <div className="process-buttom">
              <div className="cancle-order" onClick={this.Cancel.bind(this,item.payOrderNo)} >
                取消订单
              </div>
              {item.showRepayBotton ? (
                  channelid == 3 ? (
                  <div className="confirm-bank" onClick={this.confirm.bind(this,item.payOrderNo)}>
                    支付
                  </div>
                ) : (
                    <div className="confirm" onClick={this.confirm.bind(this,item.payOrderNo)}>
                      支付
                    </div>
                  )
              ):''}
        ...
        <div className={["popop_model_box",showPopup? "popup_show":""].join(' ')}>
            <div className="popop_main_content">
             ...
             { showPopup&&showFaultPop&&<div className="popup_footer">
                <p className={["cancel_btn", channelid==3? "greenBtn":""].join(' ')}  onClick={()=>{this.setState({ showPopup: false})}}>否</p>
               <p className={["determine_btn", channelid==3? "greenBtn":""].join(' ')} onClick={this.cancelOrder.bind(this,payOrderNo)}>是</p>
              </div>
              }
                { showPopup&&!showFaultPop&&<div className="popup_footer">
                <p className={["determine_btn", channelid==3? "greenBtn":""].join(' ')} onClick={this.closeModel}>确定</p>
              </div>
              }
            </div>
            </div>
            ...
            <div className="popop_main_content">
              <div className="content_title">温馨提示</div>
             <div className="popup_content bottom_line">是否取消订单</div>
            ...
            {/* <div className={["popop_model_box",showFaultPop? "popup_show":""].join(' ')}>
            <div className="popop_main_content">
              <div className="content_title">温馨提示</div>
              <div className="popup_content bottom_line">{message?message:'请求失败'}</div>
              <div className="popup_footer">
                <p className={["determine_btn", channelid==3? "greenBtn":""].join(' ')} onClick={this.closeModel}>确定</p>
              </div>
            </div>
...
SchoolPaymentProcess.propTypes = {
  nextPageStartDirection: PropTypes.func,
};

export default SchoolPaymentProcess;
