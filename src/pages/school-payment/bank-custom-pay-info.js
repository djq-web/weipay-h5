...
import '@assets/css/bank-pay-info.less';
    ...
    if (showModel) {
      this.setState({
       ...
       this.props.nextPageStartDirection('left', () => {
          navigateTo('/school-batch-pay', `billNo=${billNo}&channelid=3`);
      })
    }
  }
  render() {
    const { payInfo, isSure, isOpenHB, hubList, period, checkedAlipay, isSelfPay, isAliPay, show, showPopup, tips} = this.state;
    let list = payInfo.customData || [];
    let payAmount = payInfo.payAmount || 0;
    return (
      <div className="bankPayInfoContainer">
        <div className="page_list_header">
            <p className="header_title" onClick={()=> goBack()}>
                {
                  isSelfPay==1?<span>自助缴费</span>:<span>学号缴费</span>
                }
            </p>   
        </div>
        <div className="payment_container">
            <div className="money_box">
                  <p className="pay_title1">缴费金额</p>
                  <p className="pay_title2">￥<em className="mony_num">{(payAmount / 100).toFixed(2)}</em></p>
            </div> 
            <ul className="pay_ul_list">
              ...
              </li>
              {
                  list.length > 0 && list.map((item, index) => {
                    return (
                      <li className="payment_item_li" key={index}>
          ...
          <div className="payment_tips">
            {isSelfPay == 1 &&
              <div className="ensure_msg">
                <Checkbox onChange={(e) => this.setState({ isSure: e.target.checked })}>
                  <span className="msg_cont">请确认缴费人<span className="font_green">收费单位、缴费项目、姓名、缴费金额</span>无误</span>
                </Checkbox>
              </div>
            }
        ...
        <div className={["popop_model_box",showPopup? "popup_show":""].join(' ')}>
            <div className="popop_main_content">
              ...
              <div className="popup_footer">
                <p className="determine_btn greenBtn" onClick={this.closePopup}>确定</p>
              </div>
            </div>
        </div> 
      </div>
