...
class OtherMchPayInfo extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentWillMount() {
    ...
    this.setState({ payInfo, isSelfPay, isSure, isWeiXin, isAliPay, themeColor: payInfo.themeColor }, () => {
      if (isAliPay){
        this.checkOpenHuaBei(payInfo.commOrgId);
  ...
  toPay = () => {
    ...
    let { commOrgId,payItem,payItemList,payAmount,customData,remark,phoneNumber,billNoList, commUname } = payInfo;
    let payUrl = '';
    let reqType = 1;
    let parmas = {};
    if (!isResponse){
    ...
    if(isSelfPay === 0){
      ...
      parmas= {
        ...
        if (payInfo.isActive === 'N'){
          payUrl = API.ohterSelfPay; 
          parmas.payItem = payItem;
        }else{
          reqType = 2;
          payUrl = API.otherSelfPayItems;
  ...
  checkOpenHuaBei = (orgId) => {
    Post(API.isOpenHB, { orgType: 2, orgId }, true, 1).then(res => {
      const data = res && res.data;
      if (data) {
        const { hbStatus } = data;
        if (hbStatus === 'Y') {
  ...
  render() {
    const { payInfo, isSure, isOpenHB, hubList, period, checkedAlipay, isSelfPay, isAliPay, clock, themeColor } = this.state;
    let list = payInfo.customData || [];
    let payAmount = payInfo.payAmount || 0;
    const otherMchInfo = storage.get('receivingUnit');
    let mercPayBase  = otherMchInfo.mercPayBase || {} ;
    return (
      <div className="payment_information">
        ...
        <div className="payment_container">
            <div className="payment_top">
              ...
              <p className={`payment_amount ${themeColor==1? 'redFont': ''}`}>￥<em className="number">{(payAmount / 100).toFixed(2)}</em></p>
            </div>
            <ul className="payment_info_list">
              <li className="payment_item_li bottom_line">
                <p className="payment_label">{payInfo.commOrgId==30? "党组织名称":"收费单位"}</p>
                <p className="payment_name">{payInfo.payName}</p>
              </li>
              { payInfo.commUname && <li className="payment_item_li bottom_line">
                <p className="payment_label">{mercPayBase.userNameDesc? mercPayBase.userNameDesc: '姓名'}</p>
                <p className="payment_name">{payInfo.commUname}</p>
              </li>
              }
              { payInfo.phoneNumber && <li className="payment_item_li bottom_line">
                <p className="payment_label">{mercPayBase.userNoDesc?mercPayBase.userNoDesc: '手机号'}</p>
                <p className="payment_name">{payInfo.phoneNumber}</p>
              </li>
              }
              <li className="payment_item_li bottom_line">
                <p className="payment_label">{mercPayBase.payItemDesc?mercPayBase.payItemDesc: '缴费项目'}</p>
        ...
        <div className="payment_footer">
          {isSelfPay === 1 &&
            <div className={["ensure_msg", themeColor==1? 'redBox': ''].join(' ')}>
          ...
          <div className="payment_btn_box">
            <button className={["payment_btn", !isSure ? 'disable_btn' : '', themeColor==1? 'redBtn': ''].join(' ')} disabled={!isSure} onClick={this.toPay}>
              支付
            </button>
          </div>
        </div>
