...
class OtherMchPayInput extends Component {
  constructor(props) {
    super(props);
    this.state = {
      otherMchInfo: {},
  ...
  componentWillMount() {
    ...
    if (commOrgId){
      ...
      this.getItemOption(commOrgId);
      if (!isActive){
        // 此情况 针对某一机构只设置自助缴费的情况下用户直接扫码进入该页面
        this.checkActive(commOrgId);
      }else{
  ...
  //获取自助缴费项目列表
  getItemOption = (commOrgId) => {
      const requestUrl = API.otherMchItemInfo;
      let parmas = { commOrgId };
  ...
  checkActive = (commOrgId) => {
    Get(API.otherCheckActive, { commOrgId }, true).then((res) => {
        const data = res && res.data;
        if (data) {
          this.setState({
  ...
  getCustomData = (commOrgId) => {
    Post(API.getOtherCustomItem, { commOrgId }, true, 1).then((res) => {
        const data = res && res.data;
        if (data) {
          this.setState({
            customData: data
   ...
   * ***/
  goPayInfo = () => {
    let { commOrgId, payAmount, remark, customData, isActive, otherMchInfo, payItemList, isEdit, isOnlyOne } = this.state;
    let payName = otherMchInfo.commMercName;
    let { themeColor } = otherMchInfo;
    let payItem = this.state.payItem.replace(/(^\s*)|(\s*$)/g, "");
    if (!commOrgId) {
    ...
    this.props.nextPageStartDirection('left',()=>{
      navigateTo('/other-pay-info',`isSelfPay=1`);
    })
  }
  render() {
    const { otherMchInfo, payItem, customData, isActive, commOrgId, showPopup,payAmount, allMoney,defaultList, isEdit } = this.state;
    const { selfIn } = otherMchInfo;
    return (
      <div className="pay_form_page">
        <div className="page_main_content">
          ...
          <div className="form_container">
            <Input type="text" name="commOrgId" value={otherMchInfo.commMercName} placeholder="请输入收费单位" editable={false}>收费单位</Input>
                  ...
                  <React.Fragment key={index}>
                    {
                      item.defaultVals && item.defaultVals.length > 0 ?
                        <List.Item className={item.customValue ? 'hasContent' : ''} extra={item.customValue ? item.customValue : `请选择${item.customName}`} arrow="horizontal"
                          onClick={this.getDefaultVal.bind(this, item)} >
          ...
          <div className={`go_pay_btn ${otherMchInfo.themeColor==1? 'redBtn': ''}`}    onClick={this.goPayInfo}>缴费</div>
        </div>
        <PaymentPopup onRef={(ref) => { this.child = ref }} selectPayItmes={this.getChildValue} mercId={commOrgId} payType={3} selfIn={selfIn} onClick={this.goSelfInput} isActive={isActive} themeColor={otherMchInfo.themeColor} reduction={this.reduction} ></PaymentPopup>
        <Popup.SlidePop
