...
import schoolIcon from "@assets/image/ic_project_school@3x.png";
import schoolIconPay from "@assets/image/payment_icon.png";
class PaySelect extends Component {
  ...
  goPaymentProcess = () => {
    ...
    this.props.nextPageStartDirection("left", () => {
      navigateTo("/pay-process", `mercId=${schoolid}&userNo=${userNo}`);
    });
  };
  //查询待支付订单数量
  queryOrderNum = () => {
  ...
  getSchool = (schoolid) => {
    ...
    Get(requsetUrl, null, true).then((res) => {
      const data = res && res.data;
      if (data) {
  ...
  goPayInfo = () => {
    ...
    this.props.nextPageStartDirection("left", () => {
      navigateTo("/school-pay-info", `isSelfPay=0&minipgpay=${minipgpay}`);
    });
  };
  handleCheckPayHistory = () => {
    ...
    this.props.setEnterTimes(2);
    this.props.nextPageStartDirection("left", () => {
      navigateTo("/school-pay-history", `mercId=${schoolid}&userNo=${userNo}`);
    });
  };
  // 身份切换
  goPayBefore = () => {
  ...
  render() {
    ...
    const { schPayBase } = schoolInfo;
    // const messageSub = getQueryValue('messageSub') || '0'; // 判断是不是从消息订阅跳转过来的
    return (
      <div className="sch_select_container">
        <div className="pay_select">
          <div className="select_top">
            <div className="userName">
              <img src={schoolIcon} className="payMentImg" />
              <span>{stuName}</span>
            </div>
            <p className="info_line">
              班级：
              <span className="info_val">{classesName}</span>
            </p>
            <p className="info_line">
              {schPayBase && schPayBase.userNo ? schPayBase.userNo : "学号"}：
              <span className="info_val">{userNo}</span>
            </p>
          ...
          {!noData ? (
            <div className="page_mian_content">
              <div className="scrollContent">
                  ...
                  <List>
                    {paymentList.map((item) => (
                      <Checkbox.Item
                        ...
                        disabled={item.billPayType === 0}
                        checked={item.checked}
                      >
                        <div>
                          <span className={["itemName",item.billPayType === 0?"greyBtn":""].join(' ')}>
                            {item.payItem}
                            {item.billPayType === 0 && (
                              <span className="handle">(处理中)</span>
                            )}
                          </span>
                          <span className={["money",item.billPayType === 0?"greyBtn":""].join(' ')}>
                ...
                <div className="query_btn" onClick={this.handleCheckPayHistory}>
                  缴费历史查询
                </div>
              </div>
              {numTip > 0 && (
...
const Connects = connect(mapStateToProps, mapDispatchToProps)(PaySelect);
